...
class PolicyValueNet:
    def __init__(self, model_file=None):
        ...
        # 定义输入层
        self.input_states = tf.placeholder(tf.float32, shape=[None, 14, self.board_width, self.board_height])
        ...
        self.mcts_probs = tf.placeholder(tf.float32, shape=[None, self.prob_size])
    ...
    def policy_value_fn(self, board):
        ...
        :return:
        """
        legal_positions = board.get_legal_move_label()
        legal_id = board.get_legal_move_id()
        current_state = np.ascontiguousarray(board.current_state().reshape(-1, 14, self.board_width, self.board_height))
        act_probs, value = self.policy_value(current_state)
        act_probs = zip(legal_positions, act_probs[0][legal_id])
        return act_probs, value
