...
def rollout_policy_fn(board):
    ...
    # rollout randomly
    actions = board.get_legal_moves(board.state, board.current_player)
...
class MCTS(object):
    ...
    def _playout(self, board):
        ...
        while(1):
            ...
            board.state = board.sim_do_action(action, board.state)
            board.current_player = "w" if board.current_player == "b" else "b"
    ...
    def _evaluate_rollout(self, board, limit=1000):
        ...
        for i in range(limit):
            ...
            board.state = board.sim_do_action(max_action, board.state)
        else:
            # If no break from the loop, issue a warning.
            print("WARNING: rollout reached move limit")
        # board.print_borad(board.state)
        if winner == 't':  # tie
            return 0
        else:
...
class MCTSPlayer(object):
    ...
    def get_action(self, board):
        ...
        print("game_end {} winner {}".format(game_end, winner))
        if game_end == False:
            move = self.mcts.get_move(board)
            # actions = board.get_legal_moves(board.state, board.current_player)
            # print('1 {} {}'.format(board.current_player, actions))
            # action_id = random.randint(0, len(actions))
            # print('action_id {}'.format(action_id))
            # move = actions[action_id]
            print('move {}'.format(move))
            board.draw_board(move)
            self.mcts.update_with_move(-1)
